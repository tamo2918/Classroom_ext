(() => {
  ...
  function init() {
    ...
    installAttachmentDownloadEnhancer();
  ...
  function installAttachmentDownloadEnhancer() {
    if (PROVIDER !== "classroom" || window.top !== window) {
      return;
    }

    injectAttachmentDownloadStyles();
    scheduleAttachmentEnhancement("initial");
    window.addEventListener("pageshow", () => scheduleAttachmentEnhancement("pageshow"));
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      scheduleAttachmentEnhancement("mutation");
    });
  ...
  function scheduleAttachmentEnhancement(_reason) {
    if (PROVIDER !== "classroom" || window.top !== window) {
      return;
    }

    window.clearTimeout(state.attachmentTimer);
    state.attachmentTimer = window.setTimeout(() => {
      enhanceAttachmentDownloadButtons().catch((error) => console.warn("[CLT] attachment enhancement failed", error));
    }, 300);
  }

  async function enhanceAttachmentDownloadButtons() {
    const settings = await getSettings();
    if (!settings.showAttachmentDownloadButtons) {
      removeAttachmentDownloadButtons();
      return;
    }

    injectAttachmentDownloadStyles();
    for (const anchor of findClassroomAttachmentLinks()) {
      attachDownloadButton(anchor);
    }
  }

  function removeAttachmentDownloadButtons() {
    document.querySelectorAll(".clt-attachment-download-button").forEach((button) => button.remove());
  ...
  function isLikelyClassroomAttachment(anchor) {
    ...
    const isCardSized = rect.width >= 120 && rect.height >= 36;
    ...
    const hasFileText = /\.(pdf|docx?|xlsx?|pptx?|zip|csv|txt|mp4|mov|m4a)\b/i.test(text) ||
      /(PDF|Google ドキュメント|Google スプレッドシート|Google スライド|Microsoft Word|Microsoft Excel|Microsoft PowerPoint|Document|Spreadsheet|Presentation)/i.test(text);

    return isCardSized && opensExternally && hasFileText;
  }

  function attachDownloadButton(anchor) {
    const host = findAttachmentButtonHost(anchor);
    if (!host || host.querySelector(".clt-attachment-download-button")) {
      anchor.dataset.cltAttachmentDownloadReady = "1";
      return;
    }
    ...
    button.title = `${title || "添付資料"}をダウンロード`;
    ...
    button.addEventListener("click", (event) => {
      ...
      downloadClassroomAttachment(anchor, button).catch((error) => {
        console.warn("[CLT] attachment download failed", error);
        showToast(error.message || "添付資料のダウンロードに失敗しました", false);
      });
    }, true);

    host.classList.add("clt-attachment-download-host");
    anchor.classList.add("clt-attachment-download-source");
    anchor.dataset.cltAttachmentDownloadReady = "1";
  ...
  async function downloadClassroomAttachment(anchor, button) {
    const title = getAttachmentTitle(anchor);
  ...
  function findAttachmentButtonHost(anchor) {
    ...
    for (let depth = 0; element && depth < 4; depth += 1, element = element.parentElement) {
      ...
      const containsOnlySmallScope = rect.width >= anchorRect.width && rect.height >= anchorRect.height && rect.height <= anchorRect.height + 80;
      if (containsOnlySmallScope) {
        return element;
      }
    }
  ...
  function isAttachmentRelatedMutation(mutation) {
    ...
    for (const node of mutation.addedNodes || []) {
      if (nodeLooksAttachmentRelated(node)) {
        return true;
      }
    }
    return false;
  ...
  function nodeLooksAttachmentRelated(node) {
    ...
    if (node.matches?.("a[href], .clt-attachment-download-button")) {
      return true;
    }
  ...
  function injectAttachmentDownloadStyles() {
    if (document.getElementById("clt-attachment-download-styles")) {
      return;
    }
  ...
  function installUrlObserver() {
    window.setInterval(() => {
      ...
      scheduleAttachmentEnhancement("url-change");
    }, 800);
  }
