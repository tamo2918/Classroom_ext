...
このリポジトリは、Classroom の中で繰り返し発生する作業を小さく自動化していくためのものです。現在は、動画の文字起こしコピー、添付資料の直接ダウンロード補助、期限が近い課題の見落とし防止に対応しています。

## 現在の機能

- Google Classroom から開いた Google Drive 動画を検出
...
- Classroom ホームで提出期限が近い課題のあるクラスカードを強調表示
- 提出期限が近い課題のあるクラスカードをホーム上部に自動整列
- 提出期限が近い課題を検出したときに Chrome のシステム通知を送信
- 拡張アイコンのメニューから、各機能をシンプルなオン / オフで切り替え
...
2. Drive の動画ページで文字起こしが検出されると、自動でコピーされます。
3. Drive の mp4 では、動画の再生後に「文字起こし」ボタンが有効になることがあります。その場合は拡張が再スキャンし、サイドバー表示後にコピーします。

文字起こしコピーは、細かい形式設定を持たず、動画を開いたら自動でクリップボードにコピーする挙動を標準にしています。
...
1. Classroom のホームを開く。
2. 提出期限が近い課題が表示されているクラスカードが、期限の近さに応じた色の枠で強調されます。
3. 期限が近い課題のあるクラスカードは、ホーム上部に自動で並び替えられます。
4. 検出した期限一覧は Chrome のシステム通知にも送られます。同じ内容は短時間に何度も通知しないよう抑制します。

カードの並び替えは Google Classroom 側の保存済み順序を書き換えず、ブラウザ上の表示順だけを変更します。

### 拡張機能メニュー

拡張アイコンを押すと、次の機能をオン / オフできます。
...
- 期限の近い課題の期限解析は `src/content/due-parser.js`、カード強調は `src/content/due-highlights.js` で行います。
- 期限が近いクラスカードの表示順制御は `src/content/due-card-sorter.js` で行います。
...
- ポップアップは機能単位のオン / オフだけを扱い、文字起こしの内部設定は自動コピー向けの既定値に固定します。
...
node --check src/content/due-parser.js
node --check src/content/due-card-sorter.js
node --check src/content/due-highlights.js
...
node test/due-parser.test.mjs
node test/due-card-sorter.test.mjs
